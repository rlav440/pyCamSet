...
- **Submodule URL:** `https://github.com/ColDSnit/aruco2.git`, pinned at commit `44c41e2`
  ("Complete aruco2 wrapper hardening"). This fork's three commits on top of upstream commit
  `8755035` add the Python wrapper (`python/`, a root `pyproject.toml`, and changes to
  `CMakeLists.txt`, `README.md` and `.gitignore`), add input checks to `src/aruco2.cpp` and
  `src/aruco2_fractal.cpp`, and fix an image-edge bounds check in the latter. Upstream's `main`
  has moved on since `8755035` (it was at `93bfda4` on 2026-09-16).
