...
``pycamset`` launches the graphical calibration workflow. It is the default
install, and it is a front end to :doc:`the workflow package <workflow>`: the
GUI reads settings off its forms, hands them to the same phase functions a
script would call, and writes the same runs into the same workspace. Nothing is
calculated here that cannot be calculated without a display.

.. code-block:: bash

   pycamset

If it will not start, see :ref:`troubleshooting:\`\`pycamset\`\` will not start`.

The window
==========

.. image:: /_static/gui/phase-0-data-input-light.png
   :class: only-light
   :alt: The pyCamSet main window, showing the Phase 0 Data Input tab.

.. image:: /_static/gui/phase-0-data-input-dark.png
   :class: only-dark
   :alt: The pyCamSet main window, showing the Phase 0 Data Input tab.
...
**Create Target…**
   Opens the target generator, described in :ref:`gui:Making a target`. It is a
   dialog rather than a tab because a target is drawn once, printed, and then
   used for months of runs -- it used to sit ahead of Phase 0, so every session
   opened on the one step almost no session takes.
...
The workspace defaults to ``<f_loc>/.pycamset_workspace``, a directory beside
the images that holds every run. Recent folders are remembered between
sessions.
...
.. image:: /_static/gui/phase-3-bundle-adjustment-light.png
   :class: only-light
   :alt: The Phase 3 Bundle Adjustment tab.

.. image:: /_static/gui/phase-3-bundle-adjustment-dark.png
   :class: only-dark
   :alt: The Phase 3 Bundle Adjustment tab.
...
not quite the shape it was drawn as.

Its diagnostics say how far the target moved (D4.7), how flat it stayed
(D4.9), and how the result compares against phase 3 (D4.4), which is the
number that says whether letting the shape move helped.
...
started from, the phase 1 that fed that -- so a result can be traced back to
the detections that produced it.

Figures are matplotlib cards with **Expand** and **Save PNG** buttons.
Phase 3 and Phase 4 additionally offer **Assess Calibration**, which opens the
full-size reconstruction and residual views in native matplotlib and PyVista
windows rather than embedding them.

Making a target
===============

.. image:: /_static/gui/create-target-light.png
   :class: only-light
   :alt: The Create Target dialog.

.. image:: /_static/gui/create-target-dark.png
   :class: only-dark
   :alt: The Create Target dialog.

**Create Target…** generates a printable Ccube, ChArUco, PuzzleBoard or
PuzzleBoardCube. Both halves of the form are built from what the selected
target declares about itself -- the arguments that decide what it is, and the
options that decide how it is drawn -- so the form and the validation follow
the target rather than being written out per target.
...
COLMAP ``sparse/0`` format, for use by photogrammetry and neural
reconstruction tools. It is a wrapper around
:func:`pyCamSet.utils.saving.camset_to_colmap`, which a script can call
directly.

Optimisation
============

.. image:: /_static/gui/optimisation-light.png
   :class: only-light
   :alt: The Optimisation tab.

.. image:: /_static/gui/optimisation-dark.png
   :class: only-dark
   :alt: The Optimisation tab.

Sweeps the detector's settings, scores the calibrations that result, and
retains the best. The settings offered are the ones the selected target's
detector says it can be swept over. The work runs on a background thread, so
...
The images on this page are produced by a script, so they can be refreshed
...
It renders each tab offscreen with a pinned style and palette, in both light
and dark, so the output does not depend on the machine or the desktop theme it
was run on.
