...
It buys most of the target's resolution.  Twelve squares to an edge needs 1920
ids a block at a time and 970 shared, so it fits a thousand-marker dictionary
where it did not before: 65 corners a face rather than 30.
...
class CIco2(AbstractTarget):
    ...
    def __init__(
        ...
        # round the fold onto its neighbour.
        ...
        if border_fraction < min_border:
            ...
            raise ValueError(
                ...
                f"so its border fraction must be at least "
                f"1/{2 * n_points + 1} = {quoted:.4f}; got "
                f"border_fraction={border_fraction}.")
